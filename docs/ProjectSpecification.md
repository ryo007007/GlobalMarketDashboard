...
| `GetRiskBiasScore()` | `int` | リスク志向バイアス（±5）。**FXスコアには含めない**（10.13） |
| `GetRiskBias()` | `ENUM_RISK_BIAS` | On / Flat / Off |
...
public:
   ...
   int                GetRiskBiasScore(void);   // 10.13
...
- **[3.00]** Risk Bias の検証。季節性から導いたバイアスと、実際のキャリー通貨の翌月リターンを突き合わせ、÷2 という減衰係数を実測値に置き換える（10.13.3）

---

### 10.13 リスク志向バイアス（株の季節性とFXの関係）

#### 10.13.1 前提となる観察

株が強い局面ではリスクが取られやすく、金利差を取る取引（キャリー）が乗りやすい。逆に株が崩れると、まず解消されるのがキャリーである。この関係自体は広く観察されている。

したがって「株のアノマリーが強い季節は、リスクオン寄りに見てよいのではないか」という推論は方向として正しい。

#### 10.13.2 それでもFXのスコアに足さない理由

**理由1：推論が二次・三次になる**

10.7 の月別季節性は「株価指数の季節性」という一次の観察である。そこから
「株が強い → リスクが取られる → 高金利通貨が買われる」
と進むと、仮定の上に仮定を積むことになる。1段ごとに確度は落ちる。

**理由2：高金利通貨の顔ぶれは固定ではない**

「高金利通貨 = AUD / NZD」という組み合わせは、その時点の政策金利で決まる。金利環境が変われば入れ替わる。**そしてこのエンジンは金利を一切読まない。** 暦しか読まないエンジンに「どの通貨が高金利か」を書き込むと、その値はいずれ古くなり、しかも古くなったことに誰も気付かない。

固定表にしてよいのは、暦のように変わらないものだけである。

**理由3：どの通貨が買われているかは、すでに分かっている**

GMDには Currency Strength Engine がある。実際に買われている通貨は、価格から出ている。**アノマリーが推測で埋める必要はない。**

アノマリーが足せる情報は「どの通貨か」ではなく「今はリスクを取りやすい季節か」だけである。前者を担当すると、価格が示す事実と暦が示す推測が同じ数値に混ざり、どちらが効いているか分からなくなる。

**理由4：リスクオン/オフの判定は Market Regime の仕事**

7章の Market Regime Engine は、まさに Risk ON / OFF を判定するエンジンである。同じ判断を Anomaly Engine 側でも下すと、責務が二重になる。**アノマリーは材料を出し、判定は Market Regime が下す。**

#### 10.13.3 採用した形：Risk Bias

そこで、ご指摘の観察は「FXへの加点」ではなく **独立した文脈値** として実装する。

\[
Bias = \mathrm{clip}\left( \left\lfloor \frac{Score_{equity}}{2} \right\rfloor_{0},\ -5,\ +5 \right)
\]

（\(\lfloor \cdot \rfloor_{0}\) は0方向への丸め）

| 制限 | 値 | 理由 |
|---|---|---|
| 株スコアを半分にする | ÷2 | 推論を1段経ているため確度が下がる |
| 上限 | `GMD_RISK_BIAS_CAP = 5` | アノマリー本体の ±15 より狭く取る |
| FXスコアへの加算 | **しない** | 10.13.2 の理由1〜4 |

状態は3つに丸める。±1は誤差として扱い、絶対値2以上で初めて「傾き」と呼ぶ。

| 条件 | 状態 |
|---|---|
| `Bias >= +2` | `RISK_BIAS_ON` |
| `Bias <= -2` | `RISK_BIAS_OFF` |
| それ以外 | `RISK_BIAS_FLAT` |

#### 10.13.4 表示

ランキングの下、アノマリー行の次に1行を置く。

```text
Anomaly  +5   (五十日, 月末)
Season   Bull (risk-on bias +2)
Regime   --   [2.20]
```

色は `RISK_BIAS_ON` が赤、`RISK_BIAS_OFF` が青、`FLAT` は灰である。

この並びには意図がある。**Season 行はすぐ下の Regime 行への入力である**ことを、位置で示している。使い方は次のとおり。

> 「Season が risk-on で、強弱ランキングでも豪ドルが上位にいる」→ 追い風が揃っている
>
> 「Season が risk-off なのに、強弱では豪ドルが1位」→ 季節に逆らった動き。伸びを期待しすぎない

つまり Season は**単独で売買方向を出さない**。強弱と突き合わせて初めて意味を持つ。数値をFXに足してしまうと、この「突き合わせる」という読み方ができなくなる。

#### 10.13.5 Market Regime による消費（[2.20]）

Ver2.20 で Market Regime Engine を実装する際、`GetRiskBiasScore()` を**入力の1つ**として受け取る。ただし主たる材料は価格（株・金・債券・VIXの実際の動き）であり、季節性は**同点時の傾け（tiebreaker）にとどめる**。

重みの目安は、Risk Score 全体に対して**季節性は10%以内**とする。暦で決まる値が価格を上回る影響を持ってはならない。
